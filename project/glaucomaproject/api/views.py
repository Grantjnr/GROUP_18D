...
# ResNet Model loading commented out - system now exclusively uses ViT
# try:
...
class PredictView(APIView):
    ...
    def post(self, request):
        if not VIT_LOADED:
            return Response({"error": "ViT ML Model is currently not loaded on the server."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        try:
            ...
            # System exclusively uses ViT model (ResNet model selection commented out)
            active_model = VIT_MODEL
            actual_model_used = 'ViT'

            # ResNet model selection selection commented out:
            # selected_model_type = request.data.get('model', 'ViT')
            # if selected_model_type == 'ResNet' and RESNET_LOADED:
